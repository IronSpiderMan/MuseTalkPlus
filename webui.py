...
from musetalk.utils.utils import datagen, pronounce
...
print(audio_feature_extractor.model)
...
class Avatar:
    ...
    def process_frames(
        ...
        while self.idx < video_len - 1:
            try:
                ...
                current_idx = self.idx % len(self.coord_list_cycle)
    ...
    def inference(
        ...
        if out_vid_name is not None and skip_save_images is False:
            ...
            output_vid = os.path.join(self.video_out_path, f"{out_vid_name}.mp4")  # on
...
avatar2video = {
    "avator_1": "./data/video/zack.mp4",
    "avatar_2": "./data/video/rudy.mp4"
}
...
def musetalk(speech):
    ...
    if not avatar:
        return None
    filepath = asyncio.run(pronounce(speech, "male", "US", stream=False))
...
def load_avatar(avatar_id):
    ...
    if not avatar:
        avatar = Avatar(
            avatar_id=avatar_id,
            video_path=avatar2video[avatar_id],
            bbox_shift=5,
            batch_size=4,
            preparation=False
        )
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column():
            with gr.Row():
                ...
                avatars = gr.Dropdown(os.listdir('results/avatars'))
            output = gr.Textbox()
    ...
    load_generate.click(musetalk, inputs=[speech], outputs=[video])
