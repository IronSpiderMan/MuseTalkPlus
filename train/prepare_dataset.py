...
sys.path.append('.')
from setting import VIDEO_FRAME_DIR, AUDIO_FEATURE_DIR, TMP_FRAME_DIR, TMP_AUDIO_DIR
...
def process_video(video_path="./data/video/zack.mp4"):
    ...
    makedirs(os.path.join(VIDEO_FRAME_DIR, video_name), exist_ok=True)
    makedirs(os.path.join(AUDIO_FEATURE_DIR, video_name), exist_ok=True)
    makedirs(TMP_FRAME_DIR, exist_ok=True)
    makedirs(TMP_AUDIO_DIR, exist_ok=True)
    # 提取视频帧
    ...
    # 截取脸部
    img_list = list(glob.glob(os.path.join(TMP_FRAME_DIR, video_name, "/*")))
