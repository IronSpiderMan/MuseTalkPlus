...
def main():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        unet.train()
        for step, (target_image, ref_image, masked_image, masks, audio_feature) in enumerate(train_data_loader):
            ...
            target_image = preprocess_img_tensor(target_image).to(vae.device)
            masked_image = preprocess_img_tensor(masked_image).to(vae.device)

            with (accelerator.accumulate(unet)):
                # Convert images to latent space
                latents = vae.encode(target_image.to(dtype=weight_dtype)).latent_dist.sample()  # init image
                latents = latents * vae.config.scaling_factor

                # Convert masked images to latent space
                masked_latents = vae.encode(
                    masked_image.reshape(target_image.shape).to(dtype=weight_dtype)  # masked image
                ).latent_dist.sample()
                masked_latents = masked_latents * vae.config.scaling_factor

                # Convert ref images to latent space
                ref_latents = vae.encode(
                    ref_image.reshape(target_image.shape).to(dtype=weight_dtype)  # ref image
                ).latent_dist.sample()
                ...
                if args.reconstruction:  # decode the image from the predicted latents
                    ...
                    target_image = target_image[:, :, target_image.shape[2] // 2:, :]
                    loss_lip = F.l1_loss(image_pred_img.float(), target_image.float(),
